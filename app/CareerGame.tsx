...
import {
  faArrowRight, faArrowsLeftRight, faBolt, faBullseye, faCalendarDays, faChartLine,
...
import {
  applyMicrocycle, applySeasonAging, clearMicrocycleSelection, DEVELOPMENT_SUPPORT, developmentSupportCost, emptyDevelopmentState, forecastSession,
...
import { acceptTransfer, createMarketState, generateTransferOffers, negotiateOffer, prepareWeeklyDecision, resolveContractSeason, resolveWeeklyDecision, settleCareerWeek, settleOffseasonWeek, sponsorshipDecision, type TransferOffer } from "../game/career-market";
...
import { advanceOffseasonWeek, beginOffseason, settleWeekEnergy } from "../game/season-flow";
import type {
...
export default function CareerGame({ cloudEnabled = true }: { cloudEnabled?: boolean }) {
  ...
  const startMatch = () => {
    ...
    if (!career.trainingDone) { setView("training"); return; }
    if (career.offseason) return;
    ...
    const selection = selectPlayerForMatch(squad, { position: career.player.position, ovr: calculateOvr(career.player.position, career.player.attrs), energy: career.energy, morale: career.morale, managerTrust: career.managerTrust, availability: career.availability ?? DEFAULT_AVAILABILITY }, matchSeed);
    const created = createMatch({ playerName: career.player.name, playerNumber: career.player.number, position: career.player.position, attrs: career.player.attrs, playerOvr: calculateOvr(career.player.position, career.player.attrs), energy: career.energy, morale: career.morale, managerTrust: career.managerTrust, teamStrength: playerTeam.strength, playerClub: playerTeam, opponent, forcedRole: selection.matchRole, forcedStartMinute: selection.predictedMinute ?? undefined, specialTraits: career.development?.traits ?? [] }, matchSeed);
    const openingState = !selection.willPlay ? advanceMatch(created, 90) : selection.role === "bench" ? advanceMatch(created, created.playerStartMinute) : created;
    setMatch(setMatchSpeed({ ...openingState, competitionKind: specialFixture?.kind ?? "league", competitionFixtureId: specialFixture?.fixtureId }, career.meta?.settings.defaultSpeed ?? 1));
  ...
  const finishMatch = async () => {
    ...
    const completedDevelopment = normalizeDevelopmentState(career.development);
    const currentMarket = career.market ?? createMarketState(career.clubId, career.season, match.playerOvr, seed);
    const weeklyRecovery = settleWeeklyRecovery({ state: completedDevelopment, trainingDone: career.trainingDone, appeared, role: match.playerRole, funds: career.money, weeklySalary: currentMarket.contract.weeklySalaryEur * 4.3 });
    const development = clearMicrocycleSelection(completedDevelopment);
    ...
    setCareer({
      ...
      offseason: career.week === 30 ? beginOffseason() : undefined,
      clubId: resolvedClubId, leagueId: `${updatedClub.country}-L${updatedClub.tier}`,
      energy: settleWeekEnergy(career.weekEnergyStart ?? career.energy, career.weekPlanEnergy ?? 0, weeklyRecovery.energyDelta), weekEnergyStart: undefined, weekPlanEnergy: undefined, morale: clamp(career.morale + (match.scoreHome > match.scoreAway ? 5 : match.scoreHome < match.scoreAway ? -3 : 1)),
  ...
  const chooseTraining = (training: (typeof TRAININGS)[number]) => {
    ...
    setCareer({ ...career, development: selectMicrocycleSession(development, training.id) });
  };
  ...
  const executeMicrocycle = () => {
    ...
    const development = normalizeDevelopmentState(career.development);
    const hasTraining = Boolean(development.plan.main);
    const market = career.market ?? createMarketState(career.clubId, career.season, calculateOvr(career.player.position, career.player.attrs), seed);
    ...
    const cycleId = career.offseason ? `offseason-${career.offseason.week}` : `week-${career.week}`;
    const trainingEntry = result.moneyCost > 0 ? [{ id: `development-${career.season}-${cycleId}`, season: career.season, week: career.offseason ? 0 : career.week, amountEur: -result.moneyCost / 4.3, label: `${hasTraining ? "Wsparcie rozwoju" : "Pakiet regeneracyjny"}: ${DEVELOPMENT_SUPPORT[development.plan.support].label}` }, ...market.ledger].slice(0, 100) : market.ledger;
    setCareer({ ...career, player: { ...career.player, attrs: result.attrs }, development: result.state, trainingDone: true, weekEnergyStart: career.energy, weekPlanEnergy: result.energy, trainingCount, hiddenRevealed: career.hiddenRevealed || trainingCount >= 3, energy: clamp(career.energy + result.energy), money: Math.max(0, career.money - result.moneyCost), market: { ...market, ledger: trainingEntry }, managerTrust: clamp(career.managerTrust + (hasTraining ? result.report.response === "overload" ? -.5 : .8 : 0)) });
  };

  const finishOffseasonWeek = () => {
    if (!career?.offseason || !career.trainingDone) { setView("training"); return; }
    const market = career.market ?? createMarketState(career.clubId, career.season, calculateOvr(career.player.position, career.player.attrs), seed);
    const salary = settleOffseasonWeek(market, career.season, career.offseason.week);
    const nextOffseason = advanceOffseasonWeek(career.offseason);
    setCareer({ ...career, offseason: nextOffseason, development: clearMicrocycleSelection(normalizeDevelopmentState(career.development)), trainingDone: false, weekEnergyStart: undefined, weekPlanEnergy: undefined, energy: settleWeekEnergy(career.weekEnergyStart ?? career.energy, career.weekPlanEnergy ?? 0), market: salary.market, money: career.money + Math.round(salary.amountEur * 4.3) });
    setView(nextOffseason ? "training" : "home");
  };
  ...
  const fixture = career.offseason ? null : currentFixtureForClub(world, career.clubId);
  const previewCompetitions = career.competitions ?? createCompetitions(world, career.nationality, playerOvr);
  const nationalFixture = career.offseason ? null : getPlayerNationalFixture(previewCompetitions, career.nationality, career.week);
  const specialFixture = career.offseason ? null : nationalFixture ?? getPlayerCompetitionFixture(previewCompetitions, career.clubId, playerClub.country, career.week);
  ...
  const upcomingMatchSeed = seed + career.season * 10000 + career.week * 101;
  const lineupDecision = selectPlayerForMatch(squad, { position: career.player.position, ovr: playerOvr, energy: career.energy, morale: career.morale, managerTrust: career.managerTrust, availability }, upcomingMatchSeed);
  ...
  const legacyCompletedIds = career.trainingDone && !development.plan.main && development.lastReport?.bankedProgress
    ? development.lastReport.sessionIds?.length ? development.lastReport.sessionIds : development.recentSessions.slice(-1)
    : [];
  const displayedMain = development.plan.main ?? legacyCompletedIds[0] ?? null;
  const displayedSupplementary = development.plan.supplementary ?? legacyCompletedIds[1] ?? null;
  const completedSessionCount = [displayedMain, displayedSupplementary].filter(Boolean).length;
  const displayedEnergyDelta = career.trainingDone ? development.lastReport?.energyDelta ?? developmentPreview.energyDelta : developmentPreview.energyDelta;
  const displayedMoneyCost = career.trainingDone ? development.lastReport?.moneyCost ?? developmentPreview.moneyCost : developmentPreview.moneyCost;
  const displayedInjuryRisk = career.trainingDone ? development.lastReport?.injuryRisk ?? developmentPreview.injuryRisk : developmentPreview.injuryRisk;

  return <main className="v3-career">
    <header className="v3-top"><div className="v3-brand"><div className="brand-mark">P:N:P</div><strong>PIŁKARZ: NA PEŁNEJ</strong></div><div className="v3-season">SEZON {career.season} • {career.offseason ? `PRZERWA ${career.offseason.week}/${career.offseason.totalWeeks}` : `TYDZIEŃ ${career.week}`}</div><button onClick={() => void reset()}>NOWA KARIERA</button></header>
    ...
    <section className="v3-career-grid">
      <aside className="v3-profile"><div className="v3-profile-head"><div className="v3-player-number">{career.player.number}<small>{playerClub.short}</small></div><div><p className="micro-label">{career.player.position} • {career.player.foot} noga • {career.player.style}</p><h1>{career.player.name}</h1><span>{career.age} lat • {COUNTRY_NAMES[career.nationality]}</span></div></div><p>{playerClub.name}<small>{league.name}</small></p><div className="v3-profile-ovr"><span>OVR <small>potencjał {career.player.potential}</small></span><strong>{playerOvr.toFixed(1)}</strong></div><div className="v3-bars"><label>ENERGIA <b>{Math.round(career.energy)}%</b><i><em style={{ width: `${career.energy}%` }} /></i></label><label>MORALE <b>{Math.round(career.morale)}%</b><i><em style={{ width: `${career.morale}%` }} /></i></label><label>ZAUFANIE <b>{Math.round(career.managerTrust)}%</b><i><em style={{ width: `${career.managerTrust}%` }} /></i></label></div><div className="v3-availability"><span>{availability.injuryWeeks > 0 ? `KONTUZJA ${availability.injuryWeeks} TYG.` : availability.suspendedMatches > 0 ? "ZAWIESZONY" : "GOTOWY"}</span><b>{availability.yellowCards}/5</b><b>rytm {availability.matchSharpness}%</b></div></aside>
      <section className="v3-dashboard">
        ...
        {view === "home" && <><div className="v3-view-title"><p className="micro-label">CENTRUM KARIERY</p><h2>Jedna decyzja naraz. Resztę liczy świat.</h2></div><section className="v3-hero-card"><div><p className="micro-label">NAJWAŻNIEJSZE TERAZ</p><h3>{career.offseason ? career.trainingDone ? "Tydzień przerwy zamknięty." : "Przerwa międzysezonowa też buduje zawodnika." : career.trainingDone ? "Plan wykonany. Czas sprawdzić decyzję trenera." : "Najpierw zatwierdź plan tygodnia."}</h3><p>{career.offseason ? `Tydzień ${career.offseason.week} z ${career.offseason.totalWeeks}. Możesz trenować albo przeznaczyć cały tydzień na regenerację.` : opponent ? `Rywal: ${opponent.name}, siła ${opponent.strength.toFixed(1)}. Występ zależy od OVR, formy i zaufania.` : "Terminarz czeka na kolejną kolejkę."}</p><button onClick={() => setView("training")}>{career.trainingDone ? "ZOBACZ ZAMKNIĘTY PLAN" : "USTALAM PLAN TYGODNIA"}</button></div><div className="v3-kpis"><article><FontAwesomeIcon icon={faStar} /><span>OVR</span><strong>{playerOvr.toFixed(1)}</strong></article><article><FontAwesomeIcon icon={faGaugeHigh} /><span>FORMA</span><strong>{Math.round((career.energy + career.morale) / 2)}%</strong></article><article><FontAwesomeIcon icon={faClock} /><span>MECZE</span><strong>{career.totals.matches}</strong></article><article><FontAwesomeIcon icon={faCoins} /><span>KONTO</span><strong>{career.money} zł</strong></article></div></section><section className="v3-engine-note"><FontAwesomeIcon icon={faChartLine} /><div><strong>NOWY SILNIK MECZU</strong><p>90 minut, płynny ruch piłki i 0–7 okazji gracza. Każda akcja korzysta z minigry czasowej, a wykonanie daje jakość 0–100.</p></div></section></>}
        ...
        {view === "club" && <><div className="v3-view-title"><p className="micro-label">SZATNIA • {squad.coach.formation} • {squad.coach.mentality.toUpperCase()}</p><h2>{squad.coach.name} ustala hierarchię.</h2></div><section className="v3-selection"><div className={`v3-selection-status role-${lineupDecision.role}`}><small>OSTATECZNA DECYZJA NA MECZ</small><strong>{lineupDecision.role === "starter" ? "PIERWSZY SKŁAD • ZAGRASZ" : lineupDecision.role === "bench" ? lineupDecision.willPlay ? `ŁAWKA • WEJDZIESZ W ${lineupDecision.predictedMinute}′` : "ŁAWKA • NIE WEJDZIESZ" : "POZA KADRĄ • NIE ZAGRASZ"}</strong><p>{lineupDecision.reasons.join(" • ")}</p></div><div className="v3-coach"><span>TRENER</span><b>{squad.coach.name}</b><small>rygor {squad.coach.strictness}% • rotacja {squad.coach.rotation}%</small></div></section><div className="v3-squad-table"><header><span>ZAWODNIK</span><span>POZ.</span><span>OVR</span><span>FORMA</span><span>ZDROWIE</span><span>ROLA</span></header><div className="you"><span>{career.player.name}</span><span>{career.player.position}</span><b>{playerOvr.toFixed(1)}</b><span>{Math.round((career.energy+career.morale)/2)}</span><span>{availability.injuryWeeks ? `${availability.injuryWeeks} tyg.` : "100%"}</span><strong># {lineupDecision.positionRank}</strong></div>{squad.members.sort((a,b) => b.ovr-a.ovr).map((member) => <div key={member.id}><span>{member.name}</span><span>{member.position}</span><b>{member.ovr.toFixed(1)}</b><span>{Math.round(member.form)}</span><span>{member.injuryWeeks ? `${member.injuryWeeks} tyg.` : `${Math.round(member.fitness)}%`}</span><strong>{member.squadStatus}</strong></div>)}</div></>}
        ...
        {view === "training" && <>
          <div className="v3-view-title v3-training-title"><div><p className="micro-label">PROJEKT ROZWOJU • {career.offseason ? `PRZERWA ${career.offseason.week}/${career.offseason.totalWeeks}` : `TYDZIEŃ ${career.week}`}</p><h2>{career.offseason ? "Sześć tygodni na rozwój albo odzyskanie nóg." : "Bodziec, reakcja, adaptacja. Bez pewnych cyferek."}</h2></div><div>{(Object.keys(INTENSITY) as Intensity[]).map((key) => <button key={key} className={development.plan.intensity === key ? "active" : ""} disabled={career.trainingDone} onClick={() => setCareer({ ...career, development: setDevelopmentIntensity(development, key) })}>{INTENSITY[key].label}</button>)}</div></div>
          ...
          <section className="v3-development-status">
            ...
            <article><small>{career.trainingDone ? "WYKONANY PLAN" : "PROGNOZA ROZWOJU"}</small><strong>{career.trainingDone && completedSessionCount ? `${completedSessionCount} ${completedSessionCount === 1 ? "TRENING" : "TRENINGI"} • ${development.lastReport?.responseLabel ?? "WYKONANO"}` : developmentPreview.sessions ? `${developmentPreview.range[0].toFixed(2)}–${developmentPreview.range[1].toFixed(2)} OVR` : career.trainingDone ? "TYDZIEŃ REGENERACYJNY" : "BEZ TRENINGU"}</strong><span>{career.trainingDone && completedSessionCount ? "wybrane jednostki zostały rozliczone" : developmentPreview.sessions ? "to przedział, nie obietnica" : "pełna energia z wybranego zaplecza"}</span></article>
            <article><small>BILANS TYGODNIA</small><strong className={displayedEnergyDelta < 0 ? "negative" : ""}>{displayedEnergyDelta > 0 ? "+" : ""}{displayedEnergyDelta} energii</strong><span>koszt jednostek + regeneracja sztabu</span></article>
            <article><small>BUDŻET ROZWOJU</small><strong>{displayedMoneyCost} zł</strong><span>konto: {career.money} zł</span></article>
          </section>

          <div className="v3-cycle-slots"><span><small>BODZIEC GŁÓWNY</small><b>{TRAININGS.find((item) => item.id === displayedMain)?.title ?? "bez treningu — pełna regeneracja"}</b></span><span><small>BODZIEC UZUPEŁNIAJĄCY</small><b>{TRAININGS.find((item) => item.id === displayedSupplementary)?.title ?? "opcjonalny"}</b></span><span><small>ZAPLECZE</small><b>{DEVELOPMENT_SUPPORT[development.plan.support].label}</b></span><button disabled={career.trainingDone} onClick={executeMicrocycle}>{career.trainingDone ? "PLAN ZAMKNIĘTY" : development.plan.main ? "ZAMYKAM PLAN TYGODNIA" : "ZATWIERDZAM REGENERACJĘ"}</button></div>

          <div className="v3-training-section-label"><div><span>01</span><p><b>WYBIERZ DO DWÓCH BODŹCÓW ALBO ODPOCZNIJ</b><small>Brak wyboru oznacza pełną regenerację. Pierwsze kliknięcie ustawia trening główny, drugie uzupełniający.</small></p></div><em>RYZYKO URAZU {displayedInjuryRisk}%</em></div>
          <div className="v3-trainings">{TRAININGS.map((training) => { const slot = displayedMain === training.id ? "GŁÓWNY" : displayedSupplementary === training.id ? "UZUPEŁNIAJĄCY" : null; const forecast = forecastSession(development, training, career.age, WEIGHTS[career.player.position], slot === "UZUPEŁNIAJĄCY" ? "supplementary" : "main"); const rank = [...TRAININGS].sort((a,b) => Object.entries(b.attrs).reduce((sum,[key,gain]) => sum+(gain??0)*(WEIGHTS[career.player.position][key as AttrKey]??0),0)-Object.entries(a.attrs).reduce((sum,[key,gain])=>sum+(gain??0)*(WEIGHTS[career.player.position][key as AttrKey]??0),0)).indexOf(training)+1; return <button key={training.id} className={slot ? "selected-session" : ""} disabled={career.trainingDone} onClick={() => chooseTraining(training)}><FontAwesomeIcon icon={training.icon} /><div><small>{training.category}</small><strong>{training.title}</strong><p>{training.copy}</p></div><span>{forecast.range[0].toFixed(2)}–{forecast.range[1].toFixed(2)} OVR {slot ? <em>{slot}</em> : rank <= 3 && <em>WPŁYW P{rank}</em>}</span><footer>{Object.keys(training.attrs).map((key) => <b key={key}>{ATTR_LABELS[key as AttrKey]}</b>)}<i>przełom ok. {forecast.breakthroughChance}%</i><i className="negative">{forecast.energy} energii</i></footer></button>; })}</div>
      ...
      <aside className="v3-next">{career.offseason ? <><p className="micro-label"><FontAwesomeIcon icon={faCalendarDays} /> PRZERWA MIĘDZYSEZONOWA</p><h2>Tydzień {career.offseason.week} z {career.offseason.totalWeeks}</h2><p>{career.trainingDone ? "Plan zatwierdzony. Możesz przejść do następnego tygodnia przerwy." : "Najpierw wybierz trening albo zatwierdź samą regenerację."}</p><div className="v3-offseason-progress"><i style={{ width: `${career.offseason.week / career.offseason.totalWeeks * 100}%` }} /></div><button className="v3-primary" onClick={career.trainingDone ? finishOffseasonWeek : () => setView("training")}>{career.trainingDone ? career.offseason.week === career.offseason.totalWeeks ? "ZACZYNAM NOWY SEZON" : "NASTĘPNY TYDZIEŃ PRZERWY" : "USTAL PLAN TYGODNIA"} <FontAwesomeIcon icon={faArrowRight} /></button></> : <><p className="micro-label"><FontAwesomeIcon icon={faCalendarDays} /> {specialFixture?.label ?? `KOLEJKA ${world.round}/30`}</p>{opponent ? <><div className="v3-versus"><i style={{ background: playerClub.primary }}>{playerClub.short}</i><span>VS</span><i style={{ background: opponent.primary }}>{opponent.short}</i></div><h2>{opponent.name}</h2><p>Siła rywala <strong>{opponent.strength.toFixed(1)}</strong><br />Decyzja trenera: <strong>{lineupDecision.role === "starter" ? "zagrasz od pierwszej minuty" : lineupDecision.role === "bench" ? lineupDecision.willPlay ? `wejdziesz w ${lineupDecision.predictedMinute}. minucie` : "zostaniesz na ławce" : "nie znajdziesz się w kadrze"}</strong>.</p><div className={`v3-role-preview role-${lineupDecision.role}`}>{lineupDecision.willPlay ? lineupDecision.role === "bench" ? `PEWNE WEJŚCIE • ${lineupDecision.predictedMinute}′` : "PEWNY WYSTĘP • PIERWSZY SKŁAD" : "BEZ WYSTĘPU • OD RAZU WYNIK"}</div><button className="v3-primary" onClick={career.trainingDone ? startMatch : () => setView("training")}>{career.trainingDone ? lineupDecision.willPlay ? "JADĘ NA MECZ" : "PRZEJDŹ DO WYNIKU" : "NAJPIERW PLAN TYGODNIA"} <FontAwesomeIcon icon={faArrowRight} /></button></> : <p>Brak meczu w tej kolejce.</p>}</>}</aside>
    </section>
  </main>;
}
